...
class PlayState extends MusicBeatState
{
	...
	var wiggleThing:WiggleEffect = new WiggleEffect();
	...
	override public function create()
	{
		...
		/*
			dear kadedev:
			NOBODY CARES, NOBODY CARES, NOBODY CARES. NOBODY BEEPING CAAAAAAAAAAAAAAARRRRRRRRREEEEEEES!
		*/
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (SONG.song.toLowerCase())
			{
				case "energy": {
					curStage = 'energyisle';
					var bg:FlxSprite = new FlxSprite(-100, -250).loadGraphic(Paths.image('energydrinks'));
				...
				case "downhill": {
					curStage = 'chout';
					var bg:FlxSprite = new FlxSprite(-100, -300).loadGraphic(Paths.image('checkout'));
				...
				case "danger": {
					...
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('redsky'));
					bg.setGraphicSize(Std.int(bg.width * 1.3));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					add(bg);
	...
	override public function update(elapsed:Float)
	{
		...
		//if (curSong == "danger") {
			//wiggleThing.update(Conductor.crochet);
		//}

		if (SONG.song.toLowerCase() == 'danger') // beep you moldy! this is my code now!
			{
				if (FlxG.save.data.distractions) {
					playerStrums.forEach(function(spr:FlxSprite)
					{
						spr.x += Math.sin(elapsedtime) * ((spr.ID % 2) == 0 ? 1 : -1);
						spr.x -= Math.sin(elapsedtime) * 2.5;
					});
					cpuStrums.forEach(function(spr:FlxSprite)
					{
						spr.x -= Math.sin(elapsedtime) * ((spr.ID % 2) == 0 ? 1 : -1);
